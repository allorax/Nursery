  ...
  return (
    <div className="relative flex flex-col items-center justify-end h-64 w-full">
      ...
      <div className="absolute bottom-16 flex flex-col items-center">
        {/* Stem */}
        <motion.div 
          className="w-1.5 bg-sage-dark rounded-full origin-bottom"
...
function Leaf({ side, top, delay }: { side: 'left' | 'right', top: string, delay: number }) {
  return (
    <motion.div
      ...
      className={`absolute w-6 h-4 bg-sage rounded-full ${side === 'left' ? '-left-5 -rotate-45' : '-right-5 rotate-45'}`}
      style={{ top }}
...
function Flower({ top }: { top: string }) {
  return (
    <motion.div
      ...
      className="absolute w-10 h-10 bg-terracotta rounded-full -left-4 flex items-center justify-center"
      style={{ top }}
    >
      <div className="w-4 h-4 bg-yellow-200 rounded-full blur-[1px]" />
      {/* Petals */}
      {[0, 72, 144, 216, 288].map((deg) => (
        <div 
          key={deg}
          className="absolute w-6 h-4 bg-terracotta rounded-full origin-center"
...
function VaseSVG() {
  return (
    <svg width="100" height="120" viewBox="0 0 100 120" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M20 110C20 115.523 24.4772 120 30 120H70C75.5228 120 80 115.523 80 110V90C80 70 95 60 95 40C95 20 80 0 50 0C20 0 5 20 5 40C5 60 20 70 20 90V110Z" fill="#D97757" fillOpacity="0.9"/>
